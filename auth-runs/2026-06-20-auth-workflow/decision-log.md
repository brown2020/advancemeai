...
| DEC-005 | Tighten account-existence auth error copy | `src/lib/auth-errors.ts` exposed direct `user-not-found`, `wrong-password`, `email-already-in-use`, and provider-conflict messages | Leave as deferred product/security follow-up | Central mapper now uses less revealing sign-in and account-creation copy |
