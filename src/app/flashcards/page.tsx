...
export default function FlashcardsPage() {
  ...
  const { sets: publicSets, isLoading: isPublicLoading, error: publicError, refresh: refreshPublic } =
    usePublicFlashcards();
  ...
  const starredSetIds = useMemo(() => {
    return new Set(Object.entries(starredBySetId).filter(([, v]) => Object.keys(v ?? {}).length > 0).map(([k]) => k));
  }, [starredBySetId]);
  ...
  }, [activeFolder, activeFolderId, normalizedQuery, shouldSearchCards, sortedYourSets]);
      ...
      {tab === "recent" && (
        <>
          {recentSetIds.length === 0 ? (
            ...
            <CardGrid>
              {(() => {
                // A set can exist in both lists (e.g. your public set), so dedupe by id.
